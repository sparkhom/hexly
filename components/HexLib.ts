...
class Layout
{
    ...
    public static hexCornerOffset(layout:Layout, corner:number):Point
    {
        var M:Orientation = layout.orientation;
        var size:Point = layout.size;
        var angle:number = 2.0 * Math.PI * (corner + M.start_angle) / 6;
...
class Tests
{
    ...
    public static testHexRound():void
    {
        ...
        Tests.equalHex("hex_round 2", a, Hex.round(Hex.lerp(a, b, 0.499)));
        Tests.equalHex("hex_round 3", b, Hex.round(Hex.lerp(a, b, 0.501)));
        Tests.equalHex("hex_round 4", a, Hex.round(new Hex(a.q * 0.4 + b.q * 0.3 + c.q * 0.3, a.r * 0.4 + b.r * 0.3 + c.r * 0.3, a.s * 0.4 + b.s * 0.3 + c.s * 0.3)));
        Tests.equalHex("hex_round 5", c, Hex.round(new Hex(a.q * 0.3 + b.q * 0.3 + c.q * 0.4, a.r * 0.3 + b.r * 0.3 + c.r * 0.4, a.s * 0.3 + b.s * 0.3 + c.s * 0.4)));
    }
